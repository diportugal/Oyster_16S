...
RFTM_dds17 <- phyloseq_to_deseq2(physeq_class, ~RFTM.asnum17) 
...
x = sort(x, TRUE)
RFTM_sig17$Order = factor(as.character(RFTM_sig17$Phylum), levels= names(x))
...
RFTM_DESeq2 <- ggplot(RFTM_sig17, aes(x=Genus, y=log2FoldChange, color=Phylum))+geom_point(size=3)+ 
